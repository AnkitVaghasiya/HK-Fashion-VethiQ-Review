...
try:
    # timeframe thoda broaden kiya hai taaki stable trends milen
    pytrends.build_payload(kw_list, cat=0, timeframe='now 30-d', geo='IN', gprop='')
        ...
        try:
            ...
            for row in top_df.to_dict('records'):
                if 'query' in row and row['query']:
                    # Clean the query: remove punctuation, extra spaces
                    cleaned = row['query'].strip().lower()
                    # Keep queries that are relevant / contain product types
                    seo_keywords.add(cleaned)
        except Exception as ex:
            pass

# Agar trending se kuch nahi aaya toh default keywords
if not seo_keywords:
    seo_keywords = set([
        "floral kurti", "embroidered gown", "designer palazzo", "ethnic co ord sets",
        "festive gowns", "cotton kurtis online", "latest kurti designs", "luxury ethnic wear"
    ])

seo_keywords = list(seo_keywords)

# --- Static Data for Review Generation ---

products = ["Kurti", "Tunic", "Co-ord Set", "Pant Pair", "Palazzo Pair", "Aliya Pair", "Crop Top", "Gown"]
# postfix for Hindi (hi) / Gujarati (gu) to help grammar
product_postfix = {
...
emotions_en = ["super comfy", "stylish", "affordable", "perfect fitting", "trendy", "beautiful", "unique design", "soft fabric", "amazing quality", "exquisite"]
...
starters_hi = ["", "Yaar,", "Bahen,", "Dil se bolu toh,", "Seriously,", "Kya mast collection hai,", "Wah Bahen wah,", "Bilkul sahi jagah,", "Mast laga mujhe,"]
enders_hi = ["", "Fir aaungi pakka!", "Sabko recommend karungi.", "Mummy ko bhi pasand aayi.", "Aajkal sab yahin se le rahe hain.", "Thanks HK Fashion!", "Next time friends ke sath aavungi.", "Love it!", "Full paisa vasool."]
mid_phrases_hi = ["staff full helpful tha", "collection ek number tha", "fitting ekdum mast thi", "price bhi ekdum hu sahi he", "shop ka vibe alag hai", "service fast he", "colors mast he", "quality bhi zabardast hai", "sab kuch latest collection me hai"]

starters_gu = ["", "Maja avi gay,", "Boss, ekdam mast,", "Dil thi bolu chu,", "Yaar, su collection che,", "Full satisfied,", "Ek number,", "Sachu kav to,"]
enders_gu = ["", "Pachi pan aavish.", "Badha friends ne recommend karish.", "Mummy ne khub gamyu.", "Next time family sathe aavish.", "Thanks HK Fashion!", "Full paisa vasool.", "Love it!", "Ekdam unique che."]
...
# --- Function to generate one review ---

def generate_review(lang, product, area, emoji, keyword):
    postfix = product_postfix[product][lang] if lang in ["hi", "gu"] else ""
    shop_name = "HK Fashion"

    # SEO line includes the keyword (trending) explicitly
    seo_line = random.choice([
        f"Get latest {keyword} at {shop_name} in {area}.",
        f"{area}'s go-to for {keyword} and {product.lower()}.",
        f"{shop_name} offers trending {keyword} styles in {area}.",
        f"Looking for {keyword}? {shop_name} in {area} has got you covered.",
        f"Discover {keyword} and ethnic fashion excellence in {area}."
    ])
    ...
    if lang == "en":
        ...
        review = random.choice([
            f"{starter} I bought a {product} from {shop_name} in {area}, {mid}, it's {emotion} and perfect for {keyword}. {emoji} {ender}".strip(),
            f"{starter} From {shop_name} in {area}, I got a {product} – {emotion}, {mid}, ideal for {keyword}. {emoji} {ender}".strip(),
            f"{starter} Got a {product} at {shop_name}, {area}. It's {emotion}, {mid}, best for {keyword}. {emoji} {ender}".strip()
        ])
    elif lang == "hi":
        ...
        review = random.choice([
            f"{starter} {area} se {product} {postfix} from {shop_name}, {mid}, {emotion} aur perfect hai {keyword} ke liye. {emoji} {ender}".strip(),
            f"{starter} {shop_name} ({area}) se {product} {postfix}, {emotion}, {mid}, perfect hai {keyword} ke liye. {emoji} {ender}".strip()
        ])
    elif lang == "gu":
        ...
        review = random.choice([
            f"{starter} {area} mathi {product} {postfix} from {shop_name}, {mid}, {emotion} ane {keyword} mate ekdam perfect. {emoji} {ender}".strip(),
            f"{starter} {shop_name} {area} mathi {product} {postfix}, {emotion}, {mid}, ideal {keyword} na fans mate. {emoji} {ender}".strip()
        ])

    # Combine with SEO line
    if random.random() < 0.5:
        full_review = f"{seo_line} {review}"
    else:
        full_review = f"{review} {seo_line}"
...
while len(reviews_set) < N:
    ...
    # Paraphrase English reviews occasionally
    if lang == "en" and paraphrasing_enabled and random.random() > 0.5:
        try:
            parap = paraphraser(review, max_length=70, num_return_sequences=1)[0]['generated_text']
            # Clean paraphrase if needed
            review = parap.strip()
        except Exception:
            pass
